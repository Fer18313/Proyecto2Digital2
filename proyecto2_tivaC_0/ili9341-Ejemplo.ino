...
uint8_t p2y = 110;
uint16_t ballx = 50, bally = 50;
uint16_t b_dirx = 4, b_diry = 4;
...
up |= (digitalRead (P1_UP)==LOW);
down |= (digitalRead (P1_DOWN)==LOW);
up2 |= (digitalRead (P2_UP) == LOW);
down2 |= (digitalRead (P2_DOWN) == LOW);
if (reset_game)
{
  ...
  bally=updateY;
    ...
    if(down2){
      H_line (p2x, p2y-1, P1_H, 0x00);// borra rastro al bajar player 2
      p2y+=2;
    }
    up2 = down2 = false;
    if(p2y <15) p2y = 15;
...
void gameOver(void){
  ...
  if (p1score > p2score){
    ...
    delay(200);
    LCD_Print(String(ganador1), 100, 110, 2, 0xFFE0, 0x00);
    delay(200);
    LCD_Print(String(ganador1), 110, 120, 2, 0xF800, 0x00);
    delay(2000);
    p1score = 0;
    p2score = 0;
    inicio();
    ...
    LCD_Print(String(ganador2), 90, 100, 2, 0xF800, 0x00);
    delay(200);
    LCD_Print(String(ganador2), 100, 110, 2, 0xFFE0, 0x00);
    delay(200);
